...
import bandu_stacking.ucn.lib.utils as util_
...
class Conv2d_GN_ReLUx2(nn.Module):
    """Implements a module that performs conv2d + groupnorm + ReLU + conv2d +
    groupnorm + ReLU (and a possible downsampling operation)
    ...
    def __init__(self, in_channels, out_channels, num_groups, ksize=3, stride=1):
        ...
        self.layer1 = Conv2d_GN_ReLU(
            in_channels, out_channels, num_groups, ksize=ksize, stride=stride
        )
        self.layer2 = Conv2d_GN_ReLU(
            out_channels, out_channels, num_groups, ksize=ksize, stride=stride
        )
...
class Upsample_Concat_Conv2d_GN_ReLU_Multi_Branch(nn.Module):
    """Implements a module that performs Upsample (reduction: conv2d +
    groupnorm + ReLU + bilinear_sampling) + concat + conv2d + groupnorm + ReLU
    ...
    def __init__(
        ...
        self.channel_reduction_layer = Conv2d_GN_ReLU(
            in_channels, in_channels // 2, num_groups
        )
        self.upsample = nn.Upsample(scale_factor=2, mode="bilinear")
        self.conv_gn_relu = Conv2d_GN_ReLU(
            int(in_channels // 2 * (num_encoders + 1)), out_channels, num_groups
        )

    def forward(self, x, skips):
        """Forward module.

        @param skips: a list of intermediate skip-layer torch tensors
        from each encoder
        """
        ...
        if type(v) is str:
            if v[0] == "d":
                ...
                convtrans2d = nn.ConvTranspose2d(
                    in_channels, v, kernel_size=4, stride=2, padding=1
                )
                if batch_norm:
                    layers += [
                        convtrans2d,
...
class UNet_Encoder(nn.Module):
    ...
    def build_network(self):
        """Build encoder network Uses a U-Net-like architecture."""
...
class UNet_Decoder(nn.Module):
    ...
    def build_network(self):
        """Build a decoder network Uses a U-Net-like architecture."""

        # Fusion layer
        self.fuse_layer = Conv2d_GN_ReLU(
            self.fd * 16 * self.ne, self.fd * 16, self.fd, ksize=1
        )

        # Decoding
        self.layer1 = Upsample_Concat_Conv2d_GN_ReLU_Multi_Branch(
            self.fd * 16, self.fd * 8, self.fd, self.ne
        )
        self.layer2 = Upsample_Concat_Conv2d_GN_ReLU_Multi_Branch(
            self.fd * 8, self.fd * 4, self.fd, self.ne
        )
        self.layer3 = Upsample_Concat_Conv2d_GN_ReLU_Multi_Branch(
            self.fd * 4, self.fd * 2, self.fd, self.ne
        )
        self.layer4 = Upsample_Concat_Conv2d_GN_ReLU_Multi_Branch(
            self.fd * 2, self.fd, self.fd, self.ne
        )
        ...
        self.last_conv = nn.Conv2d(
            self.fd, self.fd, kernel_size=3, stride=1, padding=1, bias=True
        )
    ...
    def forward(self, encoder_list):
        ...
        out = torch.cat(
            [x[0] for x in encoder_list], dim=1
        )  # Concatenate on channels dimension
