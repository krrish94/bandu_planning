...
from bandu_stacking.ucn.lib.networks.embedding import EmbeddingLoss
from bandu_stacking.ucn.lib.networks.utils import (
    log_softmax_high_dimension,
    softmax_high_dimension,
)

from . import resnet_dilated, unets

__all__ = [
    "seg_vgg_embedding",
    "seg_unet_embedding",
    "seg_resnet34_8s_embedding_early",
    "seg_resnet34_8s_embedding",
    "seg_resnet50_8s_embedding",
]
...
class SEGNET(nn.Module):
    ...
    def __init__(
        ...
        self.embedding_loss = EmbeddingLoss(
            alpha, delta, lambda_intra, lambda_inter, self.metric, self.normalize
        )
        ...
        if network_name == "vgg":
            # encoder
            en_layers, en_out_channels, en_output_scale = unets.make_encoder_layers(
                encoder_archs["vgg16-based-16"],
            ...
            de_layers = unets.make_decoder_layers(
                decoder_archs["d16-embedding"], de_in_channels, batch_norm=batch_norm
            )
            self.decoder = de_layers
        elif network_name == "unet":
            # encoder
            self.encoder = unets.UNet_Encoder(
                input_channels=in_channels, feature_dim=num_units
            )

            # decoder
            self.decoder = unets.UNet_Decoder(
                num_encoders=1, feature_dim=num_units, coordconv=use_coordconv
            )
        else:
            self.fcn = getattr(resnet_dilated, network_name)(
                num_classes=num_units,
                input_channels=in_channels,
                pretrained=self.embedding_pretrain,
            )
            if self.input_type == "RGBD" and self.fusion_type != "early":
                self.fcn_depth = getattr(resnet_dilated, network_name)(
                    num_classes=num_units, input_channels=in_channels, pretrained=False
                )
