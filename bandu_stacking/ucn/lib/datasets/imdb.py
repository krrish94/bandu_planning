...
class imdb(object):
    ...
    def _build_background_images(self):
        ...
        if cfg.TRAIN.SYN_BACKGROUND_SPECIFIC:
            # NVIDIA
            """Allencenter = os.path.join(self.cache_path,
            '../AllenCenter/data') subdirs = os.listdir(allencenter) for i in
            xrange(len(subdirs)):

            subdir = subdirs[i] files =
            os.listdir(os.path.join(allencenter, subdir)) for j in
            range(len(files)):     filename = os.path.join(allencenter,
            subdir, files[j])     backgrounds_color.append(filename)
            """
            ...
            """# SUN 2012 root = os.path.join(self.cache_path,
            '../SUN2012/data/Images') subdirs = os.listdir(root)
        ...
        for i in xrange(len(subdirs)):
            subdir = subdirs[i]
            files = glob.glob(
                os.path.join(self.cache_path, "../Kinect", subdir, "*depth*")
            )
            for j in range(len(files)):
        ...
        print(
            "build color background images finished, {:d} images".format(
                len(backgrounds_color)
            )
        )
        print(
            "build depth background images finished, {:d} images".format(
                len(backgrounds_depth)
            )
        )
