...
def array_to_tensor(array):
    """Converts a numpy.ndarray (N x H x W x C) to a torch.FloatTensor of shape
    (N x C x H x W) OR converts a nump.ndarray (H x W x C) to a
...
def translate(img, tx, ty, interpolation=cv2.INTER_LINEAR):
    ...
    @param img: a [H x W x C] image (could be an RGB image, flow image,
    or label image)
    """
    H, W = img.shape[:2]
...
def add_noise_to_depth(depth_img, noise_params):
    ...
    multiplicative_noise = np.random.gamma(
        noise_params["gamma_shape"], noise_params["gamma_scale"]
    )
...
def add_noise_to_xyz(xyz_img, depth_img, noise_params):
    """Add (approximate) Gaussian Process noise to ordered point cloud. This is
    adapted from the DexNet 2.0 codebase.
    ...
    small_H, small_W = (np.array([H, W]) / noise_params["gp_rescale_factor"]).astype(
        int
    )
    additive_noise = np.random.normal(
        loc=0.0, scale=noise_params["gaussian_scale"], size=(small_H, small_W, C)
    )
...
def standardize_image(image):
    """Convert a numpy.ndarray [H x W x 3] of images to [0,1] range, and then
    standardizes.
...
def random_color_warp(image, d_h=None, d_s=None, d_l=None):
    """Given an RGB image [H x W x 3], add random hue, saturation and
    luminosity to the image.
...
def random_horizontal_flip(image, label):
    ...
    @param image: a [H x W x 3] numpy array @param label: a [H x W]
    numpy array
    """
...
def random_ellipses(label):
    ...
    while not valid_transform:
        ...
        mu = np.mean(
            pixel_indices[h_idx, w_idx, :], axis=0
        )  # Shape: [2]. y_center, x_center
        ...
        ellipse_centers = np.random.multivariate_normal(
            mu, sigma, size=num_ellipses
        )  # Shape: [num_ellipses x 2]
        ellipse_centers = np.round(ellipse_centers).astype(int)

        # Sample ellipse radii and angles
        x_min, y_min, x_max, y_max = util_.mask_to_tight_box(new_label)
        scale_factor = (
            max(x_max - x_min, y_max - y_min) * cfg.TRAIN.ellipse_size_percentage
        )  # Mean of gamma r.v.
        x_radii = np.random.gamma(
            cfg.TRAIN.ellipse_gamma_base_shape * scale_factor,
...
def random_translation(label):
    ...
    while not valid_transform:
        ...
        translation_percentage = np.random.beta(
            cfg.TRAIN.translation_alpha, cfg.TRAIN.translation_beta
        )
        translation_percentage = max(
            translation_percentage, cfg.TRAIN.translation_percentage_min
        )
...
def random_rotation(label):
    ...
    while not valid_transform:
        ...
        # Sample an angle
        applied_angle = np.random.uniform(
            -cfg.TRAIN.rotation_angle_max, cfg.TRAIN.rotation_angle_max
        )
...
def random_cut(label):
    ...
    while not valid_transform:
        ...
        cut_percentage = np.random.uniform(
            cfg.TRAIN.cut_percentage_min, cfg.TRAIN.cut_percentage_max
        )
        x_min, y_min, x_max, y_max = util_.mask_to_tight_box(label)
...
def random_add(label):
    ...
    while not valid_transform:
        ...
        add_percentage = np.random.uniform(
            cfg.TRAIN.add_percentage_min, cfg.TRAIN.add_percentage_max
        )
        x_min, y_min, x_max, y_max = util_.mask_to_tight_box(label)
        ...
        if np.random.rand() < 0.5:  # choose x direction
            ...
            )  # mask will be moved to the left/right. up/down doesn't matter
            ...
            if np.random.rand() < 0.5:  # mask copied from the left.
                ...
                try:
                    temp = added_label[
                        y_min + ty : y_max + 1 + ty, x + tx : x_max + 1 + tx
                    ]
                    added_label[
                        y_min + ty : y_max + 1 + ty, x + tx : x_max + 1 + tx
                    ] = np.logical_or(
            ...
            )  # mask will be moved up/down. lef/right doesn't matter
            ...
            if np.random.rand() < 0.5:  # from the top
                ...
                try:
                    temp = added_label[
                        y + ty : y_max + 1 + ty, x_min + tx : x_max + 1 + tx
                    ]
                    added_label[
                        y + ty : y_max + 1 + ty, x_min + tx : x_max + 1 + tx
                    ] = np.logical_or(
