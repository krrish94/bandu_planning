...
# My libraries
from bandu_stacking.ucn.lib.utils import munkres as munkres
...
def seg2bmap(seg, width=None, height=None):
    ...
    The boundary pixels are offset by 1/2 pixel towards the
...
def boundary_overlap(predicted_mask, gt_mask, bound_th=0.003):
    ...
    bound_pix = (
        bound_th
        if bound_th >= 1
        else np.ceil(bound_th * np.linalg.norm(predicted_mask.shape))
    )
...
def multilabel_metrics(prediction, gt, obj_detect_threshold=0.75):
    """Compute Overlap and Boundary Precision, Recall, F-measure Also compute
    #objects detected, #confident objects detected, #GT objects.

    It computes these measures only of objects (2+), not background (0)
    / table (1). Uses the Hungarian algorithm to match predicted masks
    with ground truth masks.

    A "confident object" is an object that is predicted with more than
    0.75 F-measure

    @param gt: a [H x W] numpy.ndarray with ground truth masks @param
    ...
    # Overlap measures
    precision = np.sum(true_positives[idx]) / np.sum(
        prediction.clip(0, 1) == OBJECTS_LABEL
    )
