...
def parse_args():
    ...
    parser.add_argument(
        "--image_path", dest="image_path", help="path to images", default=None, type=str
    )
...
def save_data(file_rgb, out_label_refined, roi, features_crop):
    ...
    file_rgb[:-9] + 'meta.mat' scipy.io.savemat(filename, meta,
    do_compression=True) print('save data to {}'.format(filename))"""
...
if __name__ == "__main__":
    ...
    if args.pretrained_crop:
        ...
        network_crop = torch.nn.DataParallel(
            network_crop, device_ids=[cfg.gpu_id]
        ).cuda(device=cfg.device)
        network_crop.eval()
    else:
        network_crop = None
