...
from bandu_stacking.robots.panda.panda_sender import (
    capture_realsense,
    close_gripper,
    command_arm,
...
class PandaController:
    ...
    def joint_positions(self):
        ...
        if self.is_closed:
            arm_joints.update(self.robot.get_closed_positions())
        elif self.is_open:
            arm_joints.update(self.robot.get_open_positions())
        return arm_joints
    ...
    def command_gripper(self, positions):
        if list(positions.values())[1] < 0.02:
