...
from bandu_stacking.policies.planning.control import (
    follow_path,
    stall_for_duration,
    step_curve,
)
from bandu_stacking.policies.planning.entities import WORLD_BODY, ParentBody
from bandu_stacking.policies.planning.grasping import (
    control_until_contact,
    get_pregrasp,
)
...
class Trajectory(Command):
    ...
    def iterate(self, state, teleport=False, **kwargs):
        if teleport:
            set_joint_positions(
                self.body, self.joints, self.path[-1], client=self.client
            )
            return self.path[-1]
        else:
            return step_curve(
...
class CaptureImage(Command):
    ...
    def switch_client(self, robot):
        return CaptureImage(robot, captured_image=self.captured_image)
    ...
    def execute(self, controller, *args, **kwargs):
        im = self.robot.get_image()
        self.captured_image = im
        return im
...
class GroupTrajectory(Trajectory):
    ...
    def execute(self, controller, *args, **kwargs):
        print("In group trajectory execute")
        ...
        print("controller.command_group_trajectory")
