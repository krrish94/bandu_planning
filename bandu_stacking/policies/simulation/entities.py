...
from bandu_stacking.policies.simulation.utils import get_rigid_ancestor
...
def create_hollow_shapes(
    indices, width=0.15, length=0.2, height=0.1, thickness=0.01, **kwargs
):
...
def create_hollow(category, color=GREY, *args, **kwargs):
    ...
    body = create_body(collision_id, visual_id, mass=STATIC_MASS, **kwargs)
    return body


def create_bin(color=RGBA(0.2, 0.2, 0.2, 0.2), *args, **kwargs):
