...
from cri.controller import ABBController, PyfrankaController, RTDEController
...
class JoggerDialog(QDialog):
    ...
    modify its settings (axes, linear speed, angular speed, blend radius, tcp
    and coordinate frame), and control its pose and joint angles."""
    ...
    def onConnect(self):
        ...
        logger.debug(
            "JoggerDialog.onConnect(robot={}, ip={}, port={})".format(robot, ip, port)
        )

        if robot in ("abb", "ur") and (
            not isValidIPAddress(ip) or not isValidPortNumber(port)
        ):
            msg = QErrorMessage(self)
        ...
        try:
            ...
            if robot == "franka":
                ...
                self.linearSpeedEditBox.setText(
                    "{:.1f}".format(self.robot.linear_speed)
                )
                self.angularSpeedEditBox.setText(
                    "{:.1f}".format(self.robot.angular_speed)
                )
                self.blendRadiusEditBox.setText(
                    "{:.1f}".format(self.robot.blend_radius)
                )
    ...
    def onLinearSpeedChanged(self):
        ...
        logger.debug("JoggerDialog.onLinearSpeedChanged(speed={})".format(linearSpeed))
        if isValidNumber(linearSpeed, self.minLinearSpeed, self.maxLinearSpeed):
            ...
            msg = QErrorMessage(self)
            msg.showMessage(
                "Invalid or out of range [{}, {}] number".format(
    ...
    def onAngularSpeedChanged(self):
        ...
        logger.debug(
            "JoggerDialog.onAngularSpeedChanged(speed={})".format(angularSpeed)
        )
    ...
    def onBlendRadiusChanged(self):
        ...
        logger.debug("JoggerDialog.onBlendRadiusChanged(radius={})".format(blendRadius))
    ...
    def onTCPChanged(self):
        logger.debug(
            "JoggerDialog.onTCPChanged(tcp={})".format(self.tcpEditWidget.value)
        )
        self.robot.tcp = self.tcpEditWidget.value

    def onCoordFrameChanged(self):
        logger.debug(
            "JoggerDialog.onCoordFrameChanged(frame={})".format(
    ...
    def onPoseChanged(self):
        logger.debug(
            "JoggerDialog.onPoseChanged(pose={})".format(self.poseControlWidget.value)
        )
        self.robot.move_linear(self.poseControlWidget.value)

    def onJointAnglesChanged(self):
        logger.debug(
            "JoggerDialog.onJointAnglesChanged(angles={})".format(
    ...
    def onTabChanged(self, index):
        logger.debug(
            "JoggerDialog.onTabChanged(index={}, text={})".format(
...
class SliderControlWidget(QWidget):
    """Slider control widget with label, slider, edit box, units and
    validation."""
...
class MultiNumberEditWidget(QWidget):
    """Multiple number edit class with label, edit box, units and
    validation."""
...
class ScrollBar(QScrollBar):
    """Scroll bar widget signals non-programmatic change in scroll bar
    value."""
