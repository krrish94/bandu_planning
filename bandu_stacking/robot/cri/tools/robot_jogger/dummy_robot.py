...
class DummyABBController:
    ...
    def __init__(self, ip="192.168.125.1", port=5000):
        logger.debug("DummyABBController.__init__(ip={}, port={})".format(ip, port))
...
class DummySyncRobot:
    ...
    def coord_frame(self, frame):
        logger.debug("DummySyncRobot.coord_frame.setter(frame={})".format(frame))
    ...
    def linear_speed(self, speed):
        logger.debug("DummySyncRobot.linear_speed.setter(speed={})".format(speed))
    ...
    def angular_speed(self, speed):
        logger.debug("DummySyncRobot.angular_speed.setter(speed={})".format(speed))
    ...
    def blend_radius(self, blend_radius):
        logger.debug(
            "DummySyncRobot.blend_radius.setter(radius={})".format(blend_radius)
        )
    ...
    def move_joints(self, joint_angles):
        logger.debug("DummySyncRobot.move_joints(angles={})".format(joint_angles))

    def move_linear(self, pose):
        logger.debug("DummySyncRobot.move_linear(pose={})".format(pose))

    def move_circular(self, via_pose, end_pose):
        logger.debug(
            "DummySyncRobot.move_circular(via pose={}, end pose={})".format(
                via_pose, end_pose
            )
        )
