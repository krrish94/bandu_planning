...
from bandu_stacking.robot.cri.cri.abb.abb_client import ABBClient
from bandu_stacking.robot.cri.cri.transforms import (
    axangle2quat,
    euler2quat,
    inv_transform,
    mat2euler,
...
from bandu_stacking.robot.cri.cri.ur.rtde_client import RTDEClient
...
class RobotController(ABC):
    """Robot controller class provides a common interface to various robot
    arms.

    Poses and coordinate frames are specified using 3D Cartesian
    positions and quaternion rotations.  This makes it easy to perform
    coordinate transformations using quaternion operations.
    """
    ...
    def tcp(self):
        ...
        The TCP is specified in the output flange frame, which is
        located at the intersection of the tool flange center axis and
        the flange face, with the z-axis aligned with the tool flange
        center axis.

        tool = (x, y, z, qw, qx, qy, qz) x, y, z specify a Cartesian
        position (mm) qw, qx, qy, qz specify a quaternion rotation
        """
        pass
    ...
    def tcp(self, tcp):
        ...
        tcp = (x, y, z, qw, qx, qy, qz) x, y, z specify a Cartesian
        position (mm) qw, qx, qy, qz specify a quaternion rotation
        """
        pass
    ...
    def joint_angles(self):
        ...
        joint angles = (j0, j1, j2, j3, j4, j5, [j6]) j0, j1, j2, j3,
        j4, j5, [j6] are numbered from base to end effector and are
        measured in degrees
        """
        pass
    ...
    def pose(self):
        ...
        pose = (x, y, z, qw, qx, qy, qz) x, y, z specify a Cartesian
        position (mm) qw, qx, qy, qz specify a quaternion rotation
        """
        pass
    ...
    def move_joints(self, joint_angles):
        ...
        joint_angles = (j0, j1, j2, j3, j4, j5) j0, j1, j2, j3, j4, j5
        are numbered from base to end effector and are measured in
        degrees
        """
        pass
    ...
    def move_linear(self, pose, elbow):
        ...
        pose = (x, y, z, qw, qx, qy, qz) x, y, z specify a Cartesian
        position (mm) qw, qx, qy, qz specify a quaternion rotation elbow
        = target elbow angle for 7-DOF robot arm (optional)
        """
        pass
    ...
    def move_circular(self, via_pose, end_pose, elbow):
        ...
        via_pose, end_pose = (x, y, z, qw, qx, qy, qz) x, y, z specify a
        Cartesian position (mm) qw, qx, qy, qz specify a quaternion
        rotation elbow = target elbow angle for 7-DOF robot arm
        (optional)
        """
        pass
...
class ABBController(RobotController):
    ...
    Currently implemented using an adapted version of the Python 2
    client example in OpenABB project (
    ...
    def linear_speed(self, speed):
        ...
        try:
            ...
            self._client.set_speed(linear_speed=speed, angular_speed=20)
        else:
            self._client.set_speed(
                linear_speed=speed, angular_speed=self._angular_speed
            )
    ...
    def angular_speed(self, speed):
        ...
        try:
            ...
            self._client.set_speed(linear_speed=20, angular_speed=speed)
        else:
            self._client.set_speed(linear_speed=self._linear_speed, angular_speed=speed)
    ...
    def blend_radius(self, blend_radius):
        """Sets the robot blend radius (mm)."""
        if blend_radius == 0:
            self._client.set_zone(point_motion=True, manual_zone=(blend_radius,) * 3)
        else:
            self._client.set_zone(point_motion=False, manual_zone=(blend_radius,) * 3)
...
class RTDEController(RobotController):
    ...
    Poses and coordinate frames are specified using 3D Cartesian
    positions and quaternion rotations.  This format makes it easy to
    ...
    def move_joints_velocity(self, joints_velocity, joints_accel, return_time=None):
        ...
        Accelerate linearly in joint space and continue with constant
        joint speed. The return time is optional; if provided the
        function will return after that time, regardless of the target
        speed has been reached. If the return time is not provided, the
        function will return when the target speed is reached.

        joint_speeds = (jd0, jd1, jd2, jd3, jd4, jd5) jd0, jd1, jd2,
        jd3, jd4, jd5 are numbered from base to end effector and are
        measured in deg/s joint_accel is measured in deg/s/s (of leading
        axis) return_time is measured in secs before the function
        returns (optional)
        """
    ...
    def move_linear_velocity(self, linear_velocity, linear_accel, return_time=None):
        ...
        Accelerate linearly in Cartesian space and continue with
        constant tool speed. The return time is optional; if provided
        the function will return after that time, regardless of the
        target speed has been reached. If the return time is not
        provided, the function will return when the target speed is
        reached

        linear speed = (xd, yd, zd, axd, ayd, azd) xd, yd, zd specify a
        translational velocity (mm/s) axd, ayd, azd specify an axis-
        angle rotational velocity (deg/s) linear_accel is measured in
        mm/s/s return_time is measured in secs before the function
        returns (optional)
        """
    ...
    def move_circular(self, via_pose, end_pose, elbow=None):
        ...
        if elbow is not None:
            warnings.warn("elbow property not implemented in RTDE controller")
        self._client.move_circular(quat2axangle(via_pose), quat2axangle(end_pose))
...
class PyfrankaController(RobotController):
    ...
    def info(self):
        """Returns a unique robot identifier string."""
        return "ip: {}, server version: {}".format(
            self._ip, self._client.server_version
        )
    ...
    def tcp(self, tcp):
        ...
        self._client.stiffness_frame = (
            self._tcp[:3] * self._scale_linear,
            self._tcp[3:],
        )

    @property
    def max_trans_velocity(self):
        """Returns the maximum translational velocity of the robot TCP
        (mm/s)."""
    ...
    def max_trans_jerk(self):
        """Returns the maximum translational jerk of the robot TCP
        (mm/s/s/s)."""
    ...
    def rel_velocity(self, rel_velocity):
        """Sets the velocity to rel_velocity % of the maximum (0 < rel_velocity
        <= 1)"""
    ...
    def rel_accel(self, rel_accel):
        """Sets the acceleraton to rel_accel % of the maximum (0 < rel_accel <=
        1)"""
    ...
    def flange_pose(self):
        """Returns the current  flange pose."""
        pose = np.concatenate(self._client.current_flange_pose)
        pose[:3] /= self._scale_linear
        return pose
    ...
    @property
    def ext_torque(self):
        return self._client.ext_torque

    @property
    def ext_wrench(self):
        return self._client.ext_wrench

    @property
    def cart_collision(self):
        return self._client.cart_collision
    ...
    # State-history related, only for pyfranka
    @property
    def pose_history(self):
        """Returns the pose history."""
        return self._client.pose_history
    ...
    def set_collision_behavior(
        ...
        self._client.set_collision_behavior(
            lower_torque_thresholds_accel,
            upper_torque_thresholds_accel,
            lower_torque_thresholds_nominal,
            upper_torque_thresholds_nominal,
