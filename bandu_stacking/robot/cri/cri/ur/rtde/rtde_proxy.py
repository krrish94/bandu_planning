...
growing too large by reading packages on a background thread when the client is
idle."""

import time
from threading import Lock, Thread

import bandu_stacking.robot.cri.cri.ur.rtde.rtde as rtde


class RTDEProxy(rtde.RTDE):
    """RTDE proxy interface prevents the socket write buffer on the server from
    growing too large by reading packages on a background thread when the
    client is idle.

    When the socket write buffer on the server reaches its maximum size
    this appears to cause some package header misalignment on the
    sending side (possibly due to the sender not checking for partial
    writes to an almost-full socket buffer).
    """
