...
class Roadmap(object):
    ...
    def check_intermediate(self, v1, v2, index, collision_fn):
        if (v1, v2, index) not in self.colliding_intermediates:
            ...
            if self.colliding_intermediates[v1, v2, index]:
                ...
                self.colliding_edges[v1, v2] = self.colliding_intermediates[
                    v1, v2, index
                ]
                if REVERSIBLE:
    ...
    def check_edge(self, v1, v2, collision_fn):
        if (v1, v2) not in self.colliding_edges:
            segment = default_selector(
                self.get_path(v1, v2)
            )  # TODO: check_intermediate
    ...
    def check_path(self, path, collision_fn):
        ...
        for v1, v2 in get_pairs(path):
            intermediates.extend(
                (v1, v2, index) for index in range(len(self.get_path(v1, v2)))
            )
...
def lazy_prm(
    ...
    **kwargs,
    ...
    if meta:
        lazy_fn = lambda v1, v2: int(
            not roadmap.is_safe(v1, v2)
        )  # TODO: score by length
        # weight_fn = lazy_fn
    ...
    heuristic_fn = lambda v: (
        visited[v].g if (v in visited) else INF
    )  # TODO: lazily apply costs
    # heuristic_fn = zero_heuristic_fn
    ...
    if path is None:
        ...
        return Solution(
            path, samples, edges, roadmap.colliding_vertices, roadmap.colliding_edges
        )
    ...
    solution = [start] + list(
        flatten(roadmap.get_path(v1, v2) for v1, v2 in get_pairs(path))
    )
    return Solution(
        solution, samples, edges, roadmap.colliding_vertices, roadmap.colliding_edges
    )
    ...
    **kwargs,
    ...
    for i, params in enumerate(param_sequence):
        ...
        if (roadmap is None) or not resuse:
            ...
            roadmap.add_samples(
                [start, goal] + sample_until(sample_fn, params["num_samples"])
            )
