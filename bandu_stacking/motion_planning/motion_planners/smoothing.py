    ...
    for iteration in irange(max_iterations):
        ...
        if verbose:
            cost = compute_path_cost(
                smoothed_path, cost_fn=cost_fn
            )  # TODO: incorporate costs
            print(
...
def smooth_path(
    ...
    for iteration in irange(max_iterations):
        ...
        if (
            verbose
            and (print_frequency is not None)
            and (iteration % print_frequency == 0)
        ):
        ...
        seg_idx1, seg_idx2 = np.random.choice(
            seg_indices, size=2, replace=True, p=probabilities
        )
        ...
        new_waypoints = (
            waypoints[: i + 1] + shortcut + waypoints[j:]
        )  # TODO: reuse computation
        new_cost = compute_path_cost(new_waypoints, cost_fn=cost_fn)
        if (
            new_cost >= cost
        ):  # TODO: cost must have percent improvement above a threshold
            continue
        if not any(
            collision_fn(q)
            for q in default_selector(refine_waypoints(shortcut, extend_fn))
        ):
