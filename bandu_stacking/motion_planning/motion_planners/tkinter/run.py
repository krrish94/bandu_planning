...
def retime_path(path, collision_fn=lambda q: False, smooth=False, **kwargs):
    ...
    print(
        "Position: t={:.3f}, error={:.3E}".format(*analyze_continuity(positions_curve))
    )
    print(
...
def solve_lazy_prm(
    viewer, start, goal, sample_fn, extend_fn, collision_fn, radius=4, **kwargs
):
    ...
    red_edges = [
        (samples[v1], samples[v2])
        for (v1, v2), c in colliding_edges.items()
        if c is True
    ]
    green_edges = [
        (samples[v1], samples[v2])
...
def main(draw=True):
    ...
    parser.add_argument(
        "-a", "--algorithm", default="rrt_connect", help="The algorithm to use."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-r", "--restarts", default=0, type=int, help="The number of restarts."
    )
    parser.add_argument(
        "-s", "--smooth", action="store_true", help="When enabled, smooths paths."
    )
    parser.add_argument(
        "-t", "--time", default=1.0, type=float, help="The maximum runtime."
    )
    parser.add_argument(
        "--seed", default=None, type=int, help="The random seed to use."
    )
    ...
    with profiler(field="tottime"):  # cumtime | tottime
        ...
        for _ in range(args.restarts + 1):
            ...
            if args.draw:
                ...
                add_roadmap(
                    viewer, roadmap, color="black"
                )  # TODO: edges going backward?
            ...
            print(
                "Solutions ({}): {} | Colliding: {} | CFree: {} | Time: {:.3f}".format(
                    len(paths),
                    [
                        (len(path), round(compute_path_cost(path, cost_fn), 3))
                        for path in paths
                    ],
                    len(colliding),
                    len(cfree),
