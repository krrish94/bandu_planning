...
from .lazy_prm import create_param_sequence, lazy_prm, lazy_prm_star
from .prm import prm
from .rrt import rrt
    ...
    **kwargs,
    ...
    print(
        "Solutions ({}): {} | Time: {:.3f}".format(
            len(solutions),
            [
                (len(path), round(compute_path_cost(path, distance_fn), 3))
...
def solve_and_smooth(
    solve_fn, q1, q2, distance_fn, sample_fn, extend_fn, collision_fn, **kwargs
):
...
def solve(
    ...
    **kwargs,
    ...
    if algorithm == "prm":  # TODO: cost_fn
        ...
        path = lattice(
            start, goal, extend_fn, collision_fn, distance_fn=distance_fn, max_time=INF
        )
    else:
