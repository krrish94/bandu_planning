...
class VideoSaver(Saver):
    def __init__(self, path):
        ...
        if path is None:
            ...
            self.log_id = p.startStateLogging(p.STATE_LOGGING_VIDEO_MP4, fileName=path)
...
class ConfSaver(Saver):
    def __init__(self, body, joints=None, positions=None, client=None):
        self.body = body
        self.client = client
        if joints is None:
...
class BodySaver(Saver):
    def __init__(self, body, client=None, **kwargs):  # , pose=None):
        # if pose is None:
        #    pose = get_pose(body)
        self.body = body
        self.client = client
...
class WorldSaver(Saver):
    def __init__(self, bodies=None, client=None):
        if bodies is None:
            bodies = get_bodies(client=client)
        self.bodies = bodies
        self.client = client
...
def set_aabb_buffer(buffer=0.0, client=None, **kwargs):
...
def set_continuous_collision_penetration(penetration=0.0, client=None, **kwargs):
...
def set_continuous_collision_radius(body, link, radius=0.0, client=None, **kwargs):
    # https://github.com/bulletphysics/bullet3/blob/0e124cb2f103c40de4afac6c100b7e8e1f5d9e15/examples/pybullet/examples/experimentalCcdSphereRadius.py
    # radius of the sphere to perform continuous collision detection
    client = client or DEFAULT_CLIENT
    client.changeDynamics(int(body), int(link), ccdSweptSphereRadius=radius)
...
def set_separating_axis_collisions(enable=True):
    ...
    p.setPhysicsEngineParameter(enableSAT=int(enable))
...
def synchronize_viewer():
    ...
    p.configureDebugVisualizer(p.COV_ENABLE_SINGLE_STEP_RENDERING, True)
...
def connect(
    use_gui=True, shadows=True, color=None, width=None, height=None, mp4=None, fps=120
):
    ...
    if use_gui:
        ...
        p.configureDebugVisualizer(
            p.COV_ENABLE_TINY_RENDERER, False
        )  # TODO: does this matter?
        p.configureDebugVisualizer(p.COV_ENABLE_SHADOWS, shadows)
        p.configureDebugVisualizer(
            p.COV_ENABLE_MOUSE_PICKING, False
        )  # mouse moves meshes
        p.configureDebugVisualizer(p.COV_ENABLE_KEYBOARD_SHORTCUTS, False)
...
def set_real_time(enable=True):
    p.setRealTimeSimulation(enableRealTimeSimulation=int(enable))
...
def set_camera(
    yaw, pitch, distance, target_position=np.zeros(3), client=None, **kwargs
):
    # TODO: in degrees
    client = client or DEFAULT_CLIENT
...
def compute_view_matrix(
    target_position, distance, yaw, pitch, roll=0.0, z_up=True, client=None
):
    client = client or DEFAULT_CLIENT
...
def set_point(body, point, **kwargs):
    set_pose(body, (point, get_quat(body, **kwargs)), **kwargs)


def set_quat(body, quat, **kwargs):
    set_pose(body, (get_point(body, **kwargs), quat), **kwargs)


def set_euler(body, euler, **kwargs):
    set_quat(body, quat_from_euler(euler), **kwargs)
...
def get_joint_positions(body, joints, **kwargs):  # joints=None):
...
def modify_configuration(body, joints, positions=None, **kwargs):
    if positions is None:
...
def get_labeled_configuration(body, **kwargs):
    movable_joints = get_movable_joints(body, **kwargs)
    return dict(
        safe_zip(
...
def movable_from_joints(body, joints, **kwargs):
    movable_from_original = {
        o: m for m, o in enumerate(get_movable_joints(body, **kwargs))
    }
...
def get_joint_intervals(body, joints, **kwargs):
    return get_min_limits(body, joints, **kwargs), get_max_limits(
        body, joints, **kwargs
    )
...
def get_max_force(body, joint, **kwargs):
    # Note that this value is not automatically used. You can use maxForce in 'setJointMotorControl2'.
    return get_joint_info(body, joint, **kwargs).jointMaxForce


def get_max_forces(body, joints, **kwargs):
    return tuple(get_max_force(body, joint, **kwargs) for joint in joints)


def get_joint_q_index(body, joint, **kwargs):
    return get_joint_info(body, joint, **kwargs).qIndex


def get_joint_v_index(body, joint, **kwargs):
    return get_joint_info(body, joint, **kwargs).uIndex


def get_joint_axis(body, joint, **kwargs):
    return get_joint_info(body, joint, **kwargs).jointAxis
...
def has_link(body, name, **kwargs):
    try:
        link_from_name(body, name, **kwargs)
    except ValueError:
        return False
    return True
...
def get_rigid_clusters(body, links=None, joints=None, **kwargs):
    # TODO: organize by root link
    if links is None:
        links = get_all_links(body, **kwargs)
    if joints is None:
        joints = get_movable_joints(body, **kwargs)
    rigid_joints = set(get_joints(body, **kwargs)) - set(joints)
    edges = {get_joint_pair(body, joint, **kwargs) for joint in rigid_joints}
...
def set_joint_limits(body, link, lower, upper, **kwargs):
    ...
    set_dynamics(body, link, jointLowerLimit=lower, jointUpperLimit=upper, **kwargs)


def set_collision_margin(body, link=BASE_LINK, margin=0.0, **kwargs):
    # TODO: might only be for soft bodies
    set_dynamics(body, link, collisionMargin=margin, **kwargs)
...
def get_local_link_pose(body, joint, **kwargs):
    parent_joint = parent_link_from_joint(body, joint, **kwargs)
...
def create_plane(normal=[0, 0, 1], mass=STATIC_MASS, color=BLACK, **kwargs):
    ...
    set_texture(body, texture=None, **kwargs)  # otherwise 'plane.urdf'
    set_color(body, color=color, **kwargs)  # must perform after set_texture
    return body
...
def clone_body(body, links=None, collision=True, visual=True, client=None, **kwargs):
    ...
    new_body = client.createMultiBody(
        ...
        baseCollisionShapeIndex=clone_collision_shape(body, base_link, client=client)
        if collision
        else NULL_ID,
        baseVisualShapeIndex=clone_visual_shape(body, base_link, client=client)
        if visual
        else NULL_ID,
...
def get_mesh_data(obj, link=BASE_LINK, shape_index=0, visual=True, client=None):
    client = client or p
    ...
    return Mesh(*client.getMeshData(int(obj), linkIndex=link, flags=flags))
...
def set_texture(
    body, texture=None, link=BASE_LINK, shape_index=NULL_ID, client=None, **kwargs
):
    client = client or DEFAULT_CLIENT
    if texture is None:
        texture = NULL_ID
    return client.changeVisualShape(
        body, link, shapeIndex=shape_index, textureUniqueId=texture
    )
...
def is_aabb_degenerate(aabb, **kwargs):
    return get_aabb_volume(aabb, **kwargs) <= 0.0
...
def get_subtree_aabb(body, root_link=BASE_LINK, **kwargs):
    return aabb_union(
        get_aabbs(body, links=get_link_subtree(body, root_link, **kwargs), **kwargs)
    )
...
def get_bodies_in_region(aabb, client=None, **kwargs):
    lower, upper = aabb
...
def set_collision_mask(body, link, group, mask=0, client=None, **kwargs):
    # p.URDF_USE_SELF_COLLISION
    # p.URDF_USE_SELF_COLLISION_EXCLUDE_PARENT
    # p.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS
    client = client or DEFAULT_CLIENT
    return client.setCollisionFilterGroupMask(int(body), link, group, mask)


def set_collision_pair_mask(
    body1, body2, link1=BASE_LINK, link2=BASE_LINK, enable=True, client=None, **kwargs
):
    return client.setCollisionFilterPair(
        int(body1), link1, int(body2), link2, enableCollision=enable
    )
...
def batch_ray_collision(rays, threads=1, client=None, **kwargs):
    ...
    return [
        RayResult(*tup)
        for tup in client.rayTestBatch(
            ray_starts,
            ray_ends,
            numThreads=threads
...
def get_duration_fn(body, joints, velocities=None, norm=INF, **kwargs):
...
def get_refine_fn(body, joints, num_steps=0, **kwargs):
    ...
    def fn(q1, q2):
        ...
        for i in range(num_steps):
            ...
            # q = tuple(positions)
            q = tuple(
                wrap_positions(body, joints, positions, **kwargs)
...
def refine_path(body, joints, waypoints, num_steps, **kwargs):
    refine_fn = get_refine_fn(body, joints, num_steps, **kwargs)
    refined_path = []
...
def smooth_path(
    ...
    **kwargs
    ...
    if resolutions is None:
        resolutions = get_default_resolutions(robot, joints, **kwargs)
    collision_fn = get_collision_fn(
        robot,
        joints,
        obstacles,
...
def discretize_curve(body, joints, curve, resolutions=None, **kwargs):
    ...
    if resolutions is None:
        resolutions = get_default_resolutions(body, joints, **kwargs)
...
def stable_z_on_aabb(body, aabb, **kwargs):
    ...
    _, upper = aabb
    return (upper + extent / 2 + (get_point(body, **kwargs) - center))[2]
...
class Attachment(object):
    def __init__(self, parent, parent_link, grasp_pose, child, client=None, **kwargs):
...
def create_attachment(parent, parent_link, child, **kwargs):
    parent_link_pose = get_link_pose(parent, parent_link, **kwargs)
    child_pose = get_pose(child, **kwargs)
    grasp_pose = multiply(invert(parent_link_pose), child_pose)
    return Attachment(parent, parent_link, grasp_pose, child, **kwargs)
...
def control_joints_hold(body, joints, positions=None, **kwargs):
    configuration = modify_configuration(body, joints, positions, **kwargs)
    return control_joints(
        body, get_movable_joints(body, **kwargs), configuration, **kwargs
    )
...
def joint_controller_hold2(
    ...
    **kwargs
    ...
    movable_joints = get_movable_joints(body, client=client)
    target_positions = list(get_joint_positions(body, movable_joints, client=client))
    ...
    current_conf = get_joint_positions(body, movable_joints, client=client)
...
def compute_jacobian(
    ...
    translate, rotate = p.calculateJacobian(
        robot, link, point, positions, velocities, accelerations
    )
...
def add_parameter(name, lower=0.0, upper=1.0, initial=0.0):
    ...
    return p.addUserDebugParameter(name, lower, upper, initial)
...
def read_parameter(debug):
    return p.readUserDebugParameter(debug)
