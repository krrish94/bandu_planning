...
CLEAR_LEFT_ARM = [PI / 2, 0.0, PI / 2, -PI / 2, PI / 2, -PI / 2, 0.0]
...
def set_gripper_position(robot, arm, position, **kwargs):
    gripper_joints = get_gripper_joints(robot, arm, **kwargs)
    set_joint_positions(robot, gripper_joints, [position] * len(gripper_joints), **kwargs)
    ...
    with LockRenderer(lock=True):
        with ConfSaver(pr2):
            for iteration in range(max_iterations):
                ...
                if verbose:
                    print(
                        "Iteration: {} | Error: {:.3f} | Correction: {}".format(
...
def visible_base_generator(
    robot, target_point, base_range=(1.0, 1.0), theta_range=(0.0, 0.0)
):
    while True:
        base_from_target = unit_from_theta(np.random.uniform(0.0, 2 * np.pi))
        look_distance = np.random.uniform(*base_range)
        base_xy = target_point[:2] - look_distance * base_from_target
        base_theta = np.math.atan2(
            base_from_target[1], base_from_target[0]
